  ...
  return <main>
    ...
    <section className="production-console" aria-label="Production Console">
      <header className="console-header">
        ...
        <div className="run-context" aria-label="Run Context Bar"><div className="context-heading"><span>运行上下文 · RUN CONTEXT</span><b>{runStatus === 'ATTENTION' ? '需处理' : runStatus}</b></div><div className="context-grid"><div><span>运行 ID · Run ID</span><strong>RUN-{city.seed}-P1</strong></div><label><span>主题版本 · ThemeVersion</span><select value={theme} onChange={e => setTheme(e.target.value as ThemeName)}>{(Object.keys(themes) as ThemeName[]).map(name => <option key={name} value={name}>{name}-local-01</option>)}</select></label><div><span>状态 · Status</span><strong>{runStatus === 'ATTENTION' ? '需处理' : runStatus}</strong></div><div><span>耗时 · Elapsed</span><strong>{elapsedLabel}</strong></div><div><span>累计成本 · Total Cost</span><strong>$0.00</strong></div></div><div className="context-actions"><label className="context-toggle"><input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} /><span>主题覆盖层 · Theme overlay</span></label><button className="decision-trigger" onClick={() => setDecisionDrawerOpen(true)}>⚠ {decisions.length} 项待处理决定</button></div></div>
      </header>
      ...
      <div className={`view-surface ${view}-surface`}>
        ...
        {view === 'cost' && <section className="surface-page system-page cost-page"><div className="surface-heading"><span>系统 / SYSTEM · COST</span><h2>成本与资源</h2><p>只展示真实本地执行和已发生的资源使用，不伪造模型调用。</p></div><div className="cost-overview"><div><span>累计成本 · Total Cost</span><strong>$0.00</strong><small>仅本地回退</small></div><div><span>Token 用量 · Token Usage</span><strong>未调用</strong><small>没有模型请求</small></div><div><span>模型调用 · Model Calls</span><strong>0</strong><small>Provider 未接入</small></div><div><span>耗时 · Elapsed</span><strong>{elapsedLabel}</strong><small>当前 Run</small></div></div><div className="resource-table"><div><span>图像 API · Image API</span><b>0 · 未调用</b><small>仅 procedural preview</small></div><div><span>3D API</span><b>0 · 未调用</b><small>批准决定完成前不调用</small></div><div><span>Codex Run</span><b>0 · 未调用</b><small>没有外部 Codex 执行</small></div><div><span>Canvas 初始化 · Canvas init</span><b>{loadMs === null ? '—' : `${loadMs} ms`}</b><small>从初始化到首帧</small></div><div><span>当前帧耗时 · Current frame</span><b>{metrics === null ? '—' : `${metrics.frameMs.toFixed(1)} ms`}</b><small>实时渲染采样</small></div><div><span>预算 · Budget</span><b>未配置</b><small>没有预算规则</small></div></div><div className="cost-trend"><span>成本趋势 · Cost trend</span><strong>暂无真实调用样本</strong><small>当模型或 API 接入后，这里再记录阶段耗时与趋势。</small></div><button className="text-link" onClick={() => setView('pipeline')}>查看相关流程 →</button></section>}
