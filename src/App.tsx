...
type PerformanceSample = RenderMetrics & { capturedAt: string; view: ViewName; time: TimeOfDay; mode: 'overview' | 'mega'; quality: QualityPreset }
...
export function App() {
  ...
  const updateMetrics = useCallback((next: RenderMetrics) => {
    setMetrics(next)
    setMetricsHistory(prev => [...prev, { ...next, capturedAt: new Date().toISOString(), view, time, mode, quality }].slice(-90))
  ...
  function exportRunReport() {
    ...
    const payload = {
      reportVersion: 'local-run-report.v1',
      exportedAt: new Date().toISOString(),
      runContext: {
