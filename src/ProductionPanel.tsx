...
export function ProductionPanel({ seed, theme }: { seed: number; theme: string }) {
  ...
  return <>
    ...
    <div className="provider-status" aria-label="Provider connection status"><div><span>当前执行源</span><strong>local-template · 本地模板</strong></div><div><span>Provider 连接</span><strong className="offline">未接入</strong></div><small>真实 API 配置将在后续 Provider 接入阶段开放。</small></div>
    ...
    <div className="log-actions"><h3>Run Log · 运行记录</h3><button disabled={!events.length} onClick={exportRunLog}>导出 JSON</button></div>
